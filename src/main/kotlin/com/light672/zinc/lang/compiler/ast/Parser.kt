...
internal class Parser(
	...
	private fun expressionStmt(expression: Expr): Stmt.Expression {
		...
		val trailing = when (expression) {
			is Expr.Block ->  // TODO: add if, while, for, loop
				!match(SEMICOLON)
	...
	private fun functionStmt(): Stmt {
		...
		expect(LEFT_PAREN, "Expected function parameters.")

		val functionParams = if (match(RIGHT_PAREN))
			emptyList()
		else {
				...
				params.add(
					when (val pattern = patternOrSelf()) {
						...
						is Either.Left -> {
							...
							Stmt.Function.NormalParam(pattern.value, expectType())
						}
					}
				)
		...
		// TODO: possibly change this from a colon to an arrow '->'
		val returnType = if (match(COLON)) expectType() else null
		return Stmt.Function(
			keyword, name, genericParams, functionParams, paramClose, returnType, if (match(SEMICOLON))
	...
	private fun variableStmt(): Stmt {
		val keyword = previous
		val pattern = pattern()
		val type = if (match(COLON)) expectType() else null
	...
	private fun typeParam(name: Token): TypeParam {
		...
		val type = if (match(EQUAL)) expectType() else null
		return TypeParam(name, bounds, type)
	}

	private fun optionalTypePath() = if (match(IDENTIFIER)) typePath() else null

	private fun typePath(): ComplexPath {
		val tail = ArrayList<ComplexPath.Segment>()
	...
	private fun pathExpression(begin: Token): Expr.Path {
		...
		while (previous.type == COLON_COLON) {
			tail.add(head)
			expect(IDENTIFIER, "Expected identifier after '::'.")
			head = pathExprSegment()
		}
		return Expr.Path(begin, ComplexPath(tail, head), previous)
	}

	private fun pathExprSegment(): ComplexPath.Segment {
		val begin = previous
		return if (match(COLON_COLON)) {
	...
	private fun typePathSegment(): ComplexPath.Segment {
		val begin = previous
	...
	private fun optionalType(): Type? {
		if (match(LEFT_PAREN)) {
			val open = previous
			if (match(RIGHT_PAREN)) return TupleType(open, emptyList(), previous)
			val args = ArrayList<Type>()
			args.add(expectType())
			while (match(COMMA)) args.add(optionalType() ?: break)
			expect(RIGHT_PAREN, "Expected ')' after tuple type.")
			return TupleType(open, args, previous)
		}
		return optionalTypePath()
	}
